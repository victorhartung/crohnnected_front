...
export const GET = withAuth(async (request: NextRequest, user: any) => {
  try {
    ...
    userCounts.forEach(({ role, _count }) => {
      ...
      users[role.toLowerCase() as keyof typeof users] = _count.id
    })
    ...
    const [reportsThisWeek, newUsersThisWeek, articlesThisWeek, protocolsThisWeek, storiesThisWeek] = await Promise.all([
      prisma.report.count({
        where: {
          createdAt: {
            gte: oneWeekAgo,
