...
export async function GET(request: NextRequest) {
  try {
    ...
    // Build where conditions - only show approved reports by default
    let where: any = {
      status: query.approvedOnly ? ReportStatus.APPROVED : undefined,
      // Only include reports with valid coordinates
      lat: { not: null },
      lng: { not: null },
    };

    // Remove status filter if approvedOnly is false (but keep only approved for public view)
    if (!query.approvedOnly) {
      where.status = ReportStatus.APPROVED; // Force approved for public map view
    }
    ...
    const geoJsonFeatures = reports
      ...
      .map(report => ({
        ...
        geometry: {
          type: 'Point' as const,
          coordinates: [parseFloat(report.lng!.toString()), parseFloat(report.lat!.toString())],
        },
        properties: {
          id: report.id,
          ageAtReport: report.ageAtReport,
          severity: report.symptomSeverity,
          symptoms: Array.isArray(report.symptoms) ? report.symptoms as string[] : [],
          city: report.city,
          state: report.state,
