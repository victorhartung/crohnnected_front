...
import { MAX_PDF_SIZE, ALLOWED_PDF_MIME } from './types';
...
export const pdfDocumentSchema = z.object({
  ...
  documentSizeBytes: z.number()
    .min(1, 'File size must be greater than 0')
    .max(MAX_PDF_SIZE, `File size must be less than ${MAX_PDF_SIZE / 1024 / 1024}MB`)
    .optional(),
}).refine(
  (data) => {
...
export const createReportSchema = z.object({
  ...
  documentOriginalName: z.string().min(1, 'Original filename is required').optional(),
...
export const rejectReportSchema = z.object({
  reason: z.string().min(1, 'Rejection reason is required').max(1000, 'Reason cannot exceed 1000 characters'),
});
...
export const reportQuerySchema = z.object({
  ...
  sortBy: z.enum(['createdAt', 'updatedAt', 'ageAtReport']).default('createdAt'),
...
export const mapQuerySchema = z.object({
  ...
  symptoms: z.string().transform((val) => val.split(',').filter(Boolean)).optional(),
  severity: z.nativeEnum(SymptomSeverity).optional(),
  approvedOnly: z.string().transform((val) => val === 'true').default('true'),
});
...
export function validateBase64PDF(base64: string, maxSize: number = MAX_PDF_SIZE): { valid: boolean; error?: string; size?: number } {
  try {
